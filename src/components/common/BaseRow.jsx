...
const BaseRow = ({ title, children, onViewAll }) => {
  ...
  return (
    <div className="space-y-6 py-10 relative group/row">
      ...
      <div className="relative overflow-hidden">
        {showLeftArrow && (
          <button
            className="absolute left-0 top-0 bottom-4 z-20 w-16 bg-gradient-to-r from-zinc-950 to-transparent flex items-center justify-start pl-4 text-white opacity-0 group-hover/row:opacity-100 transition-opacity"
            onClick={() => scroll('left')}
          >
        ...
        {showRightArrow && (
          <button
            className="absolute right-0 top-0 bottom-4 z-20 w-16 bg-gradient-to-l from-zinc-950 to-transparent flex items-center justify-end pr-4 text-white opacity-0 group-hover/row:opacity-100 transition-opacity"
